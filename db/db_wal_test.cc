...
class DBWALTestWithTimestamp
 ...
 public:
  ...
  Status CreateAndReopenWithDefaultCF(
      const std::vector<std::string>& cfs, const Options& options,
      bool avoid_flush_during_recovery = false) {
    CreateColumnFamilies(cfs, options);
    return ReopenColumnFamiliesWithDefaultCF(cfs, options,
                                             avoid_flush_during_recovery);
  }

  Status ReopenColumnFamiliesWithDefaultCF(
      const std::vector<std::string>& cfs, Options ts_options,
      bool avoid_flush_during_recovery = false) {
...
TEST_P(DBWALTestWithTimestamp, RecoverAndNoFlush) {
  ...
  // read.
  ts_options.persist_user_defined_timestamps =
      test::ShouldPersistUDT(GetParam());
  ...
  do {
    ...
    ASSERT_OK(CreateAndReopenWithDefaultCF({"pikachu"}, ts_options,
                                           avoid_flush_during_recovery));
    ASSERT_EQ(GetNumberOfSstFilesForColumnFamily(db_, "pikachu"), 0U);
    ASSERT_OK(Put(1, "foo", ts1, "v1"));
    ASSERT_OK(Put(1, "baz", ts1, "v5"));

    ASSERT_OK(ReopenColumnFamiliesWithDefaultCF({"pikachu"}, ts_options,
                                                avoid_flush_during_recovery));
    ASSERT_EQ(GetNumberOfSstFilesForColumnFamily(db_, "pikachu"), 0U);
...
TEST_F(DBWALTestWithTimestamp, EnableDisableUDT) {
  ...
  bool avoid_flush_during_recovery = true;
  ASSERT_OK(CreateAndReopenWithDefaultCF({"pikachu"}, options,
                                         avoid_flush_during_recovery));
  ...
  // timestamps.
  ASSERT_OK(ReopenColumnFamiliesWithDefaultCF({"pikachu"}, options,
                                              avoid_flush_during_recovery));
