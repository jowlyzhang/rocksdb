...
void Compaction::FilterInputsForCompactionIterator() {
  ...
  for (size_t level = 1; level < num_input_levels; level++) {
    ...
    for (FileMetaData* file : inputs_[level].files) {
      ...
      // Range deletion end key is exclusive, so check it's bigger than file
      // right boundary user key.
      if (ucmp->CompareWithoutTimestamp(std::get<0>(standalone_rangedel),
