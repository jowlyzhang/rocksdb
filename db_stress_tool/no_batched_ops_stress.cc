...
class NonBatchedOpsStressTest : public StressTest {
 public:
  ...
  void TestIngestExternalFile(ThreadState* thread,
    ...
    if (!keys.empty() && s.ok() && test_standalone_range_deletion) {
      for (int64_t key = key_base;
           s.ok() && key < shared->GetMaxKey() &&
           static_cast<int32_t>(keys.size()) < FLAGS_ingest_external_file_width;
           ++key) {
    ...
    for (size_t i = 0; s.ok() && i < keys.size(); i++ ) {
    ...
    if (s.ok() && !keys.empty() && test_standalone_range_deletion) {
      ...
      // TODO(yuzhangyu): can this be saved by not creating it in the first place.
      for (PendingExpectedValue& pending_expected_value : pending_expected_values) {
        pending_expected_value.Rollback();
      }
      pending_expected_values.clear();
      pending_expected_values =
      ...
      if (s.ok()) {
        std::cout << "yuzhangyu_debug, standalone delete range start: " << start_key << " ,range end: " << end_key << std::endl;
      }
    }
    if (s.ok()) {
      ...
      if (s.ok()) {
        std::cout << "yuzhangyu_debug, ingestion for column family: " << column_family << std::endl;
      }
    }
    if (!s.ok()) {
      for (PendingExpectedValue& pending_expected_value :
